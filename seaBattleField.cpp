#include "seaBattleField.h"
	...
	//public
		...
		const int SeaBattleField::GetLastMoveIndex() const{	//Возвращает индекс поля, в который был сделан последний ход
			int temp = -1;
			if (moves.size() > 0 && moves[moves.size() - 1].size() > 0)
				if (CheckLastMove() == 3){	//Подрыв корабля
					for(int i = 0; i < moves[moves.size() - 1].size(); i++)
						if (moves[moves.size() - 1][i].prevState == field_value_mean[4] && moves[moves.size() - 1][i].newState == field_value_mean[3])	//Клетка из живой стала взорванной
